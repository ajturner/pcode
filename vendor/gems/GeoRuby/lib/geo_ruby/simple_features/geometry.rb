module GeoRuby#:nodoc:
  module SimpleFeatures
    ...
    class Geometry
      ...
      #Outputs the geometry as an Text string.
      def as_text(allow_srid=true,allow_z=true,allow_m=true)
        text = "'" + text_geometry_type 
        text << "M" if @with_m and allow_m and (!@with_z or !allow_z) #to distinguish the M from the Z when there is actually no Z... 
        text << "(" << text_representation(allow_z,allow_m) << ")'"        
      ...
      def self.kml_to_wkt(kml)
        ...
        wkt = "SRID=4326;" + wkt # ajturner added to store SRID with geometry - can assume for now KML is 4326
        return wkt 
      end
