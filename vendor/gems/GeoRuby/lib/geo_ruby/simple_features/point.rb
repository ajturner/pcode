...
module GeoRuby
  module SimpleFeatures
    ...
    class Point < Geometry
      ...
      #Bounding box centered on a point with width and height as specified
      #http://www.movable-type.co.uk/scripts/latlong.html
      def bounding_box_with_width_height(width, height, depth = 0, radius=6370997.0)
        deg_to_rad = 0.0174532925199433
        north, east, south, west  = [0, 1.5707963267949, 3.14159265358979, 4.71238898038469]
        
        lat_n = (Math.asin( Math.sin( @y*deg_to_rad ) * Math.cos( height / radius ) + 
                      Math.cos( @y*deg_to_rad ) * Math.sin( height / radius ) * Math.cos(north) )) / deg_to_rad;
        lat_s = (Math.asin( Math.sin( @y*deg_to_rad ) * Math.cos( height / radius ) + 
                      Math.cos( @y*deg_to_rad ) * Math.sin( height / radius ) * Math.cos(south) )) / deg_to_rad;
                      
        lon_w = @x + (Math.atan2(Math.sin(west)*Math.sin( width / radius )*Math.cos( @y*deg_to_rad ), 
                             Math.cos( width / radius )-Math.sin( @y*deg_to_rad )*Math.sin( @y*deg_to_rad ))) / deg_to_rad;
        lon_e = @x + (Math.atan2(Math.sin(east)*Math.sin( width / radius )*Math.cos( @y*deg_to_rad ), 
                             Math.cos( width / radius )-Math.sin( @y*deg_to_rad )*Math.sin( @y*deg_to_rad ))) / deg_to_rad;

        
        unless with_z
          [Point.from_x_y(lon_w,lat_s),Point.from_x_y(lon_e, lat_n)]
        else
          top = @z + depth
          bottom = @z - depth
          [Point.from_x_y_z(lon_w,lat_s,bottom),Point.from_x_y_z(lat_e, lat_n,top)]
        end
      end
...
# ~> -:110: parse error, unexpected '=', expecting kEND
# ~>         [west, south, east, north ] = [0, 1.5707963267949]
# ~>                                      ^
