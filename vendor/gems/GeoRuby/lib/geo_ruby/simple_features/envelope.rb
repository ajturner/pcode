module GeoRuby
  module SimpleFeatures
    ...
    class Envelope < Geometry
      ...
      #Text representation of an envelope 
      def text_representation(allow_z=true,allow_m=true)
        corners = []
        corners << lower_corner.text_representation(allow_z,allow_m)
        corners << upper_corner.text_representation(allow_z,allow_m)
      ...
      def text_geometry_type
        "BOX3D"
			...
			def as_linestring
			  # FIXED: ajturner the x/y were switched
				LineString.from_coordinates([[lower_corner.x, lower_corner.y],[lower_corner.x, upper_corner.y],[upper_corner.x, upper_corner.y],[upper_corner.x, lower_corner.y],[lower_corner.x, lower_corner.y]],@srid)
			end      
      ...
      def self.from_points(points,srid=DEFAULT_SRID,with_z=false,with_m=false)
        e = Envelope.new(srid,with_z,with_m)
